...
int main(int argc, char** argv) {
	...
	string str1 = "AAATGTGTGT";
	string str2 = "ATGGAAACGTGAAC";
	
	int ** ed_table = create_int_grid(str1.length() , str2.length() );
	int ** test ;
	char ** bt_table = create_char_grid(str1.length(), str2.length());
	
	
	test = fill_in_edit_distance_table(str1, str2, cost_table, bt_table, str1.length(),  str2.length(), ed_table);
	print_char_grid(str1.length(), str2.length(), bt_table);	
	
	
	//free the dynamic menmory
	free_int_grid(str1.length(),str2.length(), ed_table);
	free_char_grid(str1.length(), str2.length(), bt_table);
